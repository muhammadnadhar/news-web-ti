<script lang="ts">
	...
	let angkatanList = $state<AngkatanItem[]>([
		{ id: '1', nama: '2019' },
		{ id: '2', nama: '2020' },
		{ id: '3', nama: '2021' },
		{ id: '4', nama: '2022' },
	...
	let filteredList = $derived(
		angkatanList.filter((item) => item.nama.toLowerCase().includes(searchQuery.toLowerCase()))
	);
	...
	function handleAddAngkatan(e: SubmitEvent) {
		...
		angkatanList = [...angkatanList, { id: Date.now().toString(), nama: newNama }];
		newNama = '';
		isAddModalOpen = false;
	}

	function handleDelete(id: string) {
		angkatanList = angkatanList.filter((item) => item.id !== id);
	}
</script>
